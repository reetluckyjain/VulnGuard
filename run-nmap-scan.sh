...
"$NMAP" -sT -sV -oX "$XML_FILE" --max-retries 2 --host-timeout 30s "$TARGET" 2>/dev/null

if [ ! -f "$XML_FILE" ] || ! grep -q "</nmaprun>" "$XML_FILE" 2>/dev/null; then
    # Fallback: basic scan
    "$NMAP" -sT -oX "$XML_FILE" --max-retries 1 --host-timeout 30s "$TARGET" 2>/dev/null
fi
...
if [ -n "$OPEN_PORTS" ]; then
    ...
    timeout 45 "$NMAP" -sT -sV --script vuln -oX "$VULN_XML_FILE" -p "$OPEN_PORTS" --max-retries 1 --host-timeout 30s "$TARGET" 2>/dev/null || true
fi
